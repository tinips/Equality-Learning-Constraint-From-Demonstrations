...
results = {}
train_size = int((1.0 - getattr(config, 'EVAL_SPLIT', 0.2)) * pos_np.shape[0])
for arch_name, make_model, epochs, lr in archs:
    for vid, vname in loss_variants:
        model = make_model()
        pos_train, pos_eval, neg_train, neg_eval = split_train_test(pos_np, neg_np, eval_split=config.EVAL_SPLIT)
